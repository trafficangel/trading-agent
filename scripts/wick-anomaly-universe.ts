/**
 * WICK-ANOMALY UNIVERSE SCAN — extend the deep-dislocation fade (XRP-2% / DOGE-3% robust) to a BROAD
 * coin universe. The mechanism (forced retail-leverage flushes over-extend on thin/manipulable books →
 * revert) should be STRONGER on smaller/memer alts. Same hardened lens: rest deep limit X% → fade to
 * mid; realistic cost ladder; direction-null; cross-window persistence (tolerant of newer coins with
 * fewer valid 180d windows). Bybit data (run on VPS); winners get filtered to HL-listed for the runner.
 *   pnpm tsx scripts/wick-anomaly-universe.ts [tf]
 */
import { getKlines } from '../src/backtest/klines.js';
import { type Candle } from '../src/backtest/indicators.js';

// Bybit linear-perp symbols: known winners + memes + mid/large alts + efficient-major controls.
const COINS = [
  'XRP', 'DOGE', 'ADA', // known robust
  'WIF', '1000PEPE', '1000BONK', '1000FLOKI', '1000SHIB', // memes (retail-driven, thin)
  'SUI', 'APT', 'SEI', 'TIA', 'NEAR', 'ARB', 'OP', 'INJ', 'FET', 'TON', 'TRX', 'DOT', 'ATOM',
  'FIL', 'ICP', 'HBAR', 'AAVE', 'UNI', 'LDO', 'ENA', 'ORDI', 'GALA', 'SAND', 'CRV', 'PENDLE',
  'BTC', 'ETH', 'SOL', 'LINK', 'AVAX', // controls (expected: kill)
];
...
(async () => {
  console.log(`WICK-ANOMALY UNIVERSE · ${TF}m · fade deep limit→mid · up to 3×${WIN_DAYS}d · realistic 0.10% RT · null K${K}\n`);
  ...
  console.log('  (survive conservative 0.15% too = strongest. Filter these to HL-listed before the runner.)');
