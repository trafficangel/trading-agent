...
### Обязательная проверка доставки

Наличие URL в поле **не означает**, что LuxAlgo будет отправлять webhook.
Слева от URL есть отдельный checkbox — он тоже должен быть включён.
Статус alert `Active` подтверждает только работу самой стратегии, но не
активность webhook-канала.

Перед сохранением и повторно после сохранения проверить:

1. Открыт `Webhooks → Webhook 1`.
2. Checkbox слева от URL включён.
3. URL точно указывает на канонический `/webhook/luxalgo/<WEBHOOK_SECRET>`.
4. Включены все 4 события: Long, Short, Exit Long, Exit Short.
5. Payload содержит правильный хардкодированный `strategy_id` и имеет
   статус `Valid`.
6. Нажать `Save`, снова открыть alert и повторить пункты 1–5.

**Нельзя считать alert готовым**, пока повторное открытие не подтвердило
все шесть пунктов.
...
4. **Обязательный шаг:** публикую анонс в Telegram канал командой
   ...
   на лендинг.
5. Создаю ONE webhook alert в LuxAlgo с payload выше (хардкожу
   `strategy_id` под нашу запись), включаю checkbox слева от URL и все
   4 события.
6. Сохраняю, повторно открываю alert и выполняю чек-лист
   «Обязательная проверка доставки». Без этого шага добавление стратегии
   не завершено.
