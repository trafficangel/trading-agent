  ...
  return pageShell(t(lang, 'Cross‑Venue Live — лаборатория', 'Cross‑Venue Live — Lab'), `
    ...
    <div class="cv-wrap">
      ...
      <div class="cv-grid">
        ...
        <div class="cv-card"><small>${t(lang, 'Защита', 'Risk guard')}</small><b>${s.state?.stoppedReason ? 'STOP' : '−$10'}</b><em>${t(lang, 'до', 'up to')} 10× · $100 notional · max 1 · ${Number(s.state?.consecutiveLosses ?? 0)}/3 loss streak</em></div>
      </div>
      ...
      <section class="cv-panel"><h2>${t(lang, 'Правила эксперимента', 'Experiment rules')}</h2>
        ...
        <p>${t(lang, `Реальная позиция — $100 notional с плечом до 10× и только через контроль Bybit+OKX. Для каждого контракта используется максимум, разрешённый Hyperliquid, с запасом маржи 15%. Перед approval запрашивается свежий стакан; контроль исполнения — ${primaryLatency} мс, глубина не менее $500.`, `The real position is $100 notional at up to 10× and only through the Bybit+OKX control. Each contract uses the maximum leverage allowed by Hyperliquid with a 15% margin buffer. A fresh book is requested before approval; execution control is ${primaryLatency} ms with at least $500 depth.`)}</p>
      </section>
