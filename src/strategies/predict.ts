...
const STRATEGIES: StrategyDef[] = [
  ...
  },
  {
    ...
    description: [
      ...
      'Если рынок колеблется и наливаются ОБЕ ноги — мы держим обе до конца: их суммарная цена входа меньше $1, значит прибыль заперта независимо от исхода. Если налилась только одна нога — мы НИКОГДА не доводим её до резолюции с большим убытком: стоит цене этой ноги уйти против нас всего на ~5¢, мы режем её сразу (стоп-лосс), пока по ней ещё есть покупатель. Так убыток одиночной ноги ограничен ~$1 вместо потери всей ноги. Раньше выход был только в последние 30с — и у проигрышной ноги к тому моменту уже не было бида, приходилось досиживать до резолюции и терять всё; теперь этого не происходит.',
      'Главный доход мейкера — мейкер-ребейты (часть taker-комиссии ~1.56%, которую платят те, кто выкупает наши заявки) плюс награды за ликвидность. Честно: в режиме симуляции видна только торговая часть (спред против неблагоприятного отбора) — сами ребейты начисляются Polymarket вне стакана и в paper не отражаются. Их измеряем отдельно реальной проверкой.',
      'Эта страница теперь на 5-минутных рынках (переключили с 15m). Важная оговорка: программа ребейтов/наград подтверждена на 15m, а на 5m её статус другой/пилотный — то есть на 5m основной доход мейкера может и не работать. Зато paper честно покажет, насколько ХУЖЕ торговая часть на быстрых рынках: окно короче → «замок» (наливаются обе ноги) реже, а убыточная «одна нога до резолюции» — чаще. Это и проверяем; статистика сброшена под чистый 5m-замер.',
      'Откуда именно берётся PnL (по таблице). Колонка «Что» показывает, чем кончился раунд, а «Куплено / продано» — как сложилась прибыль: 🔒 Замок — выкупили ОБЕ заявки, например UP @0.06 + DOWN @0.90 = 0.96; платим $0.96 за пару, на закрытии победившая нога гасится в $1.00 → разница $0.04 на акцию заперта независимо от исхода (≈ +$0.80 на 20 акций). ✂️ Срез — налилась одна нога (например DOWN @0.95), но до закрытия наш же бид по ней подрос, и мы продали дороже (@0.99) → прибыль $0.04 на акцию со спреда, направленный риск не несём. 📥 Одна нога — налилась только одна заявка: убыточную мы режем стоп-лоссом рано (малый минус, ~$1 максимум), плюсовую фиксируем по биду у закрытия. До полного убытка ногу больше не доводим. PnL каждого раунда — в своей колонке.',
      'Важно про масштаб и честность. Прибыль с торговой части крошечная (центы со спреда на сделку). Это НОРМАЛЬНО: основной доход мейкера — ребейты и награды за ликвидность, которых в симуляции НЕТ (они начисляются вне стакана). Поэтому здесь мы лишь проверяем, что торговая часть не уходит в большой минус. Плюс на этой странице — это ещё не доказанная доходность.',
    ],
  },
];
