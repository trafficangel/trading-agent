...
const STRATEGIES: StrategyDef[] = [
  ...
  },
  {
    ...
    engine: 'endgame-even',
    realEligible: false,
    title: 'Коэф-2 · мейкер-тест 🧪',
    tagline: 'Решающая чистая проверка идеи коэф-2: пассивная мейкер-лимитка (комиссия≈0, спред не платим). После того как «+29пп» оказался артефактом измерения.',
    ...
    hasLive: true,
    description: [
      'ЧЕСТНО О ПРЕДЫСТОРИИ. Мы думали, что нашли край на коэф ~2.0 (+29пп). Строгий повторный разбор 2352 сделок (единичная ставка, дедуп раундов, out-of-sample) показал: это был АРТЕФАКТ — смесь мартингейл-стейков, двойного счёта коррелированных раундов (125 «входов» = 58 уник.) и перегона на 2 удачных днях. Честный край зоны 0.45–0.60 = −8.4пп, а вся книга −3.4% на ставку. Стабильного края в данных НЕ нашлось.',
      'ЧТО ТЕПЕРЬ ПРОВЕРЯЕМ. Остался один незакрытый механизм: вход не рыночным ордером (платит спред + комиссию ~2%), а ПАССИВНОЙ МЕЙКЕР-ЛИМИТКОЙ по best bid — комиссия GTC≈0, спред не платим. Это единственное, что теоретически может вытащить тонкий лаг в плюс. Тест отвечает: выживает ли коэф-2 после издержек при мейкер-исполнении.',
      'КАК ВХОДИТ. Пассивная лимитка на покупку по best bid (НЕ кроссим спред), фикс 5 акций (≈$2.5), зона ask 0.45–0.55, |z|≥1.0, согласие бирж. Движок наливает только если рынок реально пересёк нашу цену (как на реале — ловит и адверс-селекцию). Не налилось за раунд — пропуск, НЕ догоняем.',
      '🧪 СТАТУС: РЕШАЮЩИЙ ТЕСТ, не доказанный край. Планка: плюс ПОСЛЕ комиссий на 150+ независимых раундах, в обеих половинах выборки и в ≥5 из 7 дней. Не прошёл — края нет, тему закрываем честно. Шанс успеха низкий (~70–85%, что рынок просто эффективен). Реальные деньги — только если планка взята. realEligible=FALSE.',
    ],
  },
  {
    slug: 'mart',
