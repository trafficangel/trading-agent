...
const STRATEGIES: StrategyDef[] = [
  ...
  },
  {
    ...
    description: [
      ...
      'Когда раунд пропускаем. Нет актуального направления (направленного сигнала не было дольше 4 часов); сторона сигнала дороже 0.50 (коэф < 2); цена глубже 0.30; не хватает времени или ликвидности.',
  ...
  {
    slug: 'arb',
    title: 'Арбитраж (вилка)',
    tagline: 'Берём обе стороны разом, когда их сумма цен < $1 — безрисковый замок',
    statusEnv: 'PREDICT_ARB_STATUS_PATH',
    statusFile: 'predict-arb-status.json',
    liveFile: 'predict-arb-live.json',
    showStakeCol: false,
    hasLive: true,
    isTrap: true,
    description: [
      'Настоящая безрисковая вилка внутри одного рынка. В бинарном рынке ровно одна сторона выплатит $1. Если в один момент цена UP + цена DOWN получится меньше $1 — мы покупаем ОБЕ стороны равными долями: тратим меньше доллара, а получаем гарантированно $1, как бы ни выпало. Профит заперт = 1 − сумма цен. Пример: UP по 0.50 и DOWN по 0.476 → сумма 0.976 → +2.4% без риска.',
      'В отличие от «ценовой ловушки» (которая набирает ноги по очереди и может застрять на одной), здесь обе ноги берутся ОДНОВРЕМЕННО в один момент — направленного риска нет вообще. Входим, только если сумма цен ≤ 0.99 (запертый плюс не меньше 1¢ на акцию) и в стакане хватает объёма на обе ноги. Держим до резолюции.',
      'Честно — главный вопрос не в риске, а в частоте. На эффективном рынке цены сторон в сумме больше $1 (это спред), и вилки просто нет. Поэтому стратегия каждую минуту измеряет минимальную сумму цен за раунд и пишет её в лог — это прямо покажет, бывает ли вообще сумма < 1 на наших 5-минутных рынках. Если книга всегда ≥ 1.00 — вилок нет, и мы это честно зафиксируем (сделок будет ноль). Если бывают мгновенные провалы — поймаем их.',
    ],
  },
];
