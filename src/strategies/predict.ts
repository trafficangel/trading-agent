...
const STRATEGIES: StrategyDef[] = [
  ...
  },
  {
    ...
    description: [
      ...
      'Честно: это вариация той же гипотезы (лаг тонкого рынка против уже складывающегося исхода). Эдж не гарантирован — судим по статистике на достаточной выборке. Параметры не публикуем.',
    ],
  },
  {
    slug: 'egprog',
    title: 'Эндшпиль · догон (Фибоначчи)',
    tagline: 'Эндшпиль + прогрессия ставки по Фибоначчи: после проигрыша — следующий шаг, после выигрыша — сброс',
    ...
    hasLive: true,
    description: [
      'Вход — как у обычного эндшпиля (недооценённая почти решённая сторона в последние секунды). Отличие — в РАЗМЕРЕ ставки: используется прогрессия Фибоначчи от базы $10 (10, 10, 20, 30, 50). После проигрыша делаем следующий шаг, после выигрыша возвращаемся к базе; серия ограничена 5 шагами (потом сброс).',
      'Идея проверки: у эндшпиля высокий процент выигрышей и редкие серии проигрышей, поэтому прогрессия может ровно «доганять» нечастые минусы. Тестируем именно это предположение на данных.',
      'Честно: прогрессия НЕ создаёт математического преимущества — она перераспределяет результат во времени (много мелких плюсов и редкий крупный минус при серии). Работает только если у самого входа неотрицательное матожидание и серии проигрышей действительно редкие. Это эксперимент в симуляции; вывод — по статистике, без иллюзий.',
    ],
  },
  {
    slug: 'lux',
    title: 'LuxAlgo сигналы',
    tagline: 'Держим направление последнего Confirmation LuxAlgo (1m); вход на эту сторону при цене 0.45–0.85 (не аутсайдер)',
    ...
    description: [
      ...
      'Направление. Текущую сторону задаёт последний направленный сигнал (Confirmation Buy→UP/Sell→DOWN, либо тренд Catcher/Tracer вверх/вниз). Это направление ДЕРЖИТСЯ, пока не прилетит новый Confirmation в другую сторону — то есть мы торгуем по сигналу, пока он не сменится (а не ждём новый каждый раунд). Направление держим до 4 часов (пока не придёт новый сигнал) — ночью сигналы редкие, поэтому держим последнюю сторону дольше; если сигналов не было дольше — не торгуем. Сторону задаёт последний из Confirmation / Trend Catcher / Trend Tracer.',
      'Вход. Каждый раунд входим на сторону по нашему направлению, если её цена в полосе 0.45–0.85 (не лезем в пыль <0.30 и в почти решённые >0.85). Раньше был фильтр «только дёшево» (цена ≤0.50), но он заставлял покупать сторону, которая сейчас проигрывает (она потому и дешёвая) — это давало отрицательный результат. Теперь берём сторону сигнала по рынку, без перекоса в аутсайдера.',
      'Когда раунд пропускаем. Нет актуального направления (направленного сигнала не было дольше 4 часов); цена нужной стороны вне полосы 0.45–0.85; не хватает времени или ликвидности.',
      'Честно. Это направленная ставка по индикатору. Сигналы LuxAlgo запаздывают и могут перерисовываться, а угадывание направления на 5-минутном BTC у нас нигде не дало устойчивого плюса — поэтому прибыль НЕ гарантирована. Снятие фильтра ≤0.50 убирает покупку заведомо проигрывающей стороны, но эдж появится, только если само направление реально предсказывает исход. Проверяем строго по статистике; параметры не публикуем.',
    ],
  },
  {
    slug: 'mm',
    title: 'Маркет-мейкинг (15m)',
    tagline: 'Резящие котировки с обеих сторон ниже середины — зарабатываем на спреде и ребейтах',
    statusEnv: 'PREDICT_MM_STATUS_PATH',
    statusFile: 'predict-mm-status.json',
    liveFile: 'predict-mm-live.json',
    showStakeCol: false,
    hasLive: true,
    isTrap: true,
    description: [
...
function renderNoAccess(authed: boolean): string {
  ...
  return (
    ...
    `<li>В перспективе — подключение собственного кошелька и реальная торговля (откроется только после того, как у стратегии подтвердится устойчивый эдж).</li>` +
    `</ul>` +
