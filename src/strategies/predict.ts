...
const STRATEGIES: StrategyDef[] = [
  ...
  {
    slug: 'egprog',
    title: 'Эндшпиль · компаундинг',
    tagline: 'Эндшпиль + пропорциональная ставка от банка (дробный Келли) — плавный рост без разгона',
    ...
    hasLive: true,
    description: [
      'Вход — как у обычного эндшпиля (недооценённая почти решённая сторона в последние секунды). Отличие — в РАЗМЕРЕ ставки. Раньше тут был «догон» (повышали ставку при просадке) — мы его убрали: догон раздувает риск и ничего не чинит. Теперь система противоположная — пропорциональная.',
      'Как считается ставка. Ставим небольшую долю (≈4%) от ТЕКУЩЕГО банка (банк = старт + накопленный PnL). Банк подрос — ставка чуть выросла (капитал компаундит, растёт плавно и геометрически); банк просел — ставка САМА уменьшилась (защита). Никакого разгона на проигрышах, есть жёсткий потолок. Размер ещё взвешивается по коэффициенту входа через формулу Келли: чем сильнее перевес нашей оценки над ценой и выгоднее коэффициент — тем больше доля (но в пределах потолка); нет перевеса — ставим минимум.',
      'Почему так правильно. Это математически корректный способ растить капитал: пропорция от банка даёт устойчивый рост при положительном матожидании и автоматически страхует от разорения (ставка не может «разогнаться» в серию проигрышей).',
      'Честно: рост будет ТОЛЬКО если у самого входа эндшпиля реально положительное преимущество. Пропорция/Келли НЕ создаёт преимущества — она лишь превращает уже существующее преимущество в плавный рост и защищает от разгона. Если преимущества нет — капитал стоит на месте (а не сливается догоном). Это эксперимент в симуляции; вывод — по статистике.',
    ],
  },
  {
    slug: 'eglock',
  ...
  },
  {
    ...
    hasLive: true,
    description: [
      'Вход — как у обычного эндшпиля. Система ставки — анти-мартингейл (парлей) с капом: после выигрыша поднимаем ставку (×2), и так до 3 побед подряд — затем фиксируем прибыль и возвращаемся к базе $1. После ЛЮБОГО проигрыша — сразу база.',
  ...
  },
  {
    ...
    hasLive: true,
    description: [
      'Вход — эндшпиль. Система ставки — классический мартингейл, но с ЖЁСТКИМ капом: после проигрыша удваиваем ($1→$2→$4→$8), максимум 3 шага, дальше не растём; на выигрыше — сброс к базе $1.',
  ...
  },
  {
    ...
    hasLive: true,
    description: [
      'Вход — эндшпиль. Система ставки — Oscar’s Grind (известная низковолатильная прогрессия). Цель — заработать +$1 за «серию». Ставку повышаем (+$1) ТОЛЬКО после выигрыша; после проигрыша оставляем как есть (не догоняем). Как только серия вышла в +$1 — сброс к базе $1.',
  ...
  },
  {
    ...
    hasLive: true,
    description: [
      'Вход — эндшпиль. Система ставки — d’Alembert: уровень ставки растёт на +1 единицу после проигрыша и падает на −1 после выигрыша (минимум 1). Рост ЛИНЕЙНЫЙ ($1, $2, $3…), а не геометрический — поэтому намного мягче и безопаснее мартингейла. Кап уровня — 6 (потолок $6).',
