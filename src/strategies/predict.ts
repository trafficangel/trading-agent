...
const STRATEGIES: StrategyDef[] = [
  {
    slug: 'udk',
    engine: 'underdog-kelly',
    realEligible: false,
    title: 'Андердог коэф-3 + Келли 🧪',
    tagline: 'Честная версия мартингейла: тот же вход (андердог коэф 3 в окне 60–180с), но размер ставки по Келли. По Монте-Карло — больше роста и ноль разорений.',
    statusEnv: 'PREDICT_UDK_STATUS_PATH',
    statusFile: 'predict-udk-status.json',
    liveFile: 'predict-live-udk.json',
    showStakeCol: true,
    hasLive: true,
    description: [
      'ИДЕЯ. Под мартингейлом лежит хороший вход — ставка на андердога коэф 3.0–3.5 в окне 60–180с до закрытия. На нём разбор 559 сделок дал ~40% винрейт против безубытка 33% — это кандидат в настоящий край. Но мартингейл портит его разгоном ставки. Здесь берём ТОТ ЖЕ вход, а размер считаем по Келли.',
      'ПОЧЕМУ КЕЛЛИ, А НЕ МАРТИНГЕЙЛ. Монте-Карло (20 000 прогонов, старт $100): на крае 40% мартингейл даёт медиану $234, но в худших 5% — $12 и 7% полных разорений. ¼-Келли на том же крае: медиана $366 (БОЛЬШЕ) и НОЛЬ разорений. Без края (33%) мартингейл разоряется в 44% случаев, а Келли просто не разгоняется. Келли строго лучше по всем показателям.',
      'КАК СЧИТАЕТСЯ СТАВКА. Дробный (¼) Kelly: f = p − (1−p)/b, где b = коэф−1, p = наш фактический винрейт (после 20 сделок) либо осторожный приор 35%. Потолок ставки 10% банка, минимум $1. Растёт от банка при выигрышах, ужимается при просадке — плавный рост без обрывов.',
      '🧪 ФОРВАРД-ВАЛИДАЦИЯ. Это и есть честный путь к постоянному росту из идеи мартингейла. Крутится рядом с мартингейлами — будет видно разницу вживую. В реал — только после подтверждения края на форварде. Только бумага.',
    ],
  },
  {
    slug: 'eglate',
  ...
  },
  {
    ...
    engine: 'endgame-cal',
    realEligible: true,
    title: 'Эндшпиль · калибр (все улучшения) ⭐',
    tagline: 'Флагман: откалиброванная вероятность по нашим данным — главный фильтр края; vol-режим + антизапаздывание; работает 24/7',
    statusEnv: 'PREDICT_EGCAL_STATUS_PATH',
    statusFile: 'predict-egcal-status.json',
    liveFile: 'predict-egcal-live.json',
    showStakeCol: true,
    hasLive: true,
    description: [
      'Главное улучшение — ОТКАЛИБРОВАННАЯ ВЕРОЯТНОСТЬ. Мы разобрали 407 наших же сделок (z → реальный исход) и обнаружили, что модель Φ(z) системно переоценивает себя на 14–33 процентных пункта: при z≥1.5 модель уверяет в 92–100%, а по факту винрейт упирается в плато ~78–80%. Из-за этого старый фильтр входа (вероятность − цена ≥ 0.1) пускал в дорогие убыточные входы. Здесь подставляем РЕАЛЬНУЮ вероятность из наших данных — и фильтр сам отсекает дорогую зону.',
      'Главный фильтр входа — сам откалиброванный EV-гейт: входим, только если откалиброванная вероятность минус цена ≥ 10¢. Это автоматически отсекает дорогую зону (вход выходит при цене ≈ ≤0.68). Поэтому жёсткие ручные пороги ослаблены (z≥1.0, цена до 0.80) — решение принимает калибровка, а не сырые пороги. Вход только при положительном крае f>0. Работает 24/7 (сессионный фильтр убран — он у строгой комбо egcombo).',
      'Плюс два новых фильтра. (1) Vol-режим: торгуем только в средней полосе волатильности (не мёртвый штиль, где gap в шуме, и не взрыв, где модель ломается). (2) Антизапаздывание: не входим, если цена лидера уже резко подскочила за последние секунды — значит стакан уже переоценился и лаг (наш источник края) закрылся, мы опоздали.',
      'Размер ставки — дробный Kelly (¼) от банка на фактическом винрейте, потолок 30%, минимум $1. Это самая «выверенная» наша стратегия: каждое решение принимается по реальным данным, а не по самоуверенной формуле. Честно: даже так это заточка существующего края, а не магия — судим по выборке.',
    ],
  },
  {
    slug: 'egtwo',
    engine: 'endgame-even',
    realEligible: false,
    title: 'Коэф-2 (вход на ~2.0) 🧪',
    tagline: 'Вход ТОЛЬКО на коэффициенте ~2.0 (цена ~0.50), где разбор 1640 наших сделок дал +29 пунктов края. Идёт форвард-валидация.',
    statusEnv: 'PREDICT_EGTWO_STATUS_PATH',
    statusFile: 'predict-egtwo-status.json',
    liveFile: 'predict-egtwo-live.json',
    showStakeCol: true,
    hasLive: true,
    description: [
      'ОТКУДА ИДЕЯ — из данных, а не из головы. Мы разбили 1640 наших сделок по цене входа и нашли резкий край ровно на коэффициенте ~2.0: при цене 0.50–0.525 винрейт 80% при безубытке всего 51% — это +29 пунктов края (+$179). А на фаворитах (цена 0.80+) — стабильный минус: там winrate 78%, но безубыток 82%, мы переплачиваем. Край держится по 5 разным стратегиям (egparlay 86%, egz2 80%, egsnap 71%…), значит это не случайность одной.',
      'ПОЧЕМУ РАБОТАЕТ (уточнено по данным). Проверка 80 исторических выигрышей показала: у них был СИЛЬНЫЙ сигнал (|z| медиана 1.94), а не слабый. Механизм — ЛАГ: BTC резко двинулся у дедлайна, а тонкий стакан в последнюю минуту не успел переоценить цену, и сильная сторона секунду-две стоит у 0.50 вместо 0.80. Мы покупаем её в эту фору по равному коэффициенту. Сильный ход обычно удерживается (~80%) — выигрываем ~80%, заплатив как за монетку.',
      'КАК ВХОДИТ. Нужны ОБА условия: сильный сигнал (|z| ≥ 1.0) И цена ведущей стороны ≤0.60 (рынок ещё не переоценил). Чем дешевле поймали (ближе к 0.50) — тем больше край. Дороже 0.65 — фаворит-парадокс (минус), туда не лезем. Вероятность — откалиброванная по 407 сделкам.',
      'ДЕНЬГИ — КОНСЕРВАТИВНО (рост без большого риска): ⅛-Kelly от осторожного винрейта, потолок ставки 10% банка, минимум $1.',
      '🧪 СТАТУС: ФОРВАРД-ВАЛИДАЦИЯ. Вход РЕДКИЙ — лаг стакана бывает нечасто (несколько раз в день), это нормально для такого края, а не поломка. Стратегия копит чистую forward-статистику. Планка перед допуском к реалу: винрейт ≥70% на коэф ≥1.9 на 150+ форвард-сделках. Честно: пока это гипотеза с сильным сигналом, а не доказанный факт.',
    ],
  },
  {
    slug: 'mart',
    engine: 'martingale-fib',
    realEligible: false,
    title: 'Мартингейл-Фибоначчи (коэф 3) 🧪',
    tagline: 'Ставка на андердога коэф 3.0–3.5 + рост ставки по Фибоначчи. Исправлено по данным: окно входа 60–180с + стоп-серия на 10. Только бумага.',
    ...
    hasLive: true,
    description: [
      'Механика: ставим на аутсайдера (цена 0.286–0.333, коэффициент 3.0–3.5), после проигрыша увеличиваем ставку по Фибоначчи [1,1,2,3,5,8,13,21], после выигрыша — сброс к базе. Идея: редкие, но крупные по коэффициенту выигрыши отыгрывают серию мелких проигрышей.',
      'ИСПРАВЛЕНО ПО ДАННЫМ (разбор 559 сделок). Главная находка: исход почти полностью определяется ВРЕМЕНЕМ входа. В окне 60–180с до закрытия андердог выигрывает ~40% (при безубытке 30% — это плюс), а при входе раньше 180с — лишь 26% (минус: у BTC слишком много времени уйти к лидеру). Старая версия входила слишком рано (68% сделок) — отсюда весь минус. Теперь вход ТОЛЬКО в окне 60–180с.',
      'ЗАЩИТА ОТ СЕРИЙ. Стоп-серия на 10 проигрышах подряд: пауза 1 час + сброс ставки к базе. Физически не даёт серии и ставке разгоняться (раньше варианты доходили до 19 проигрышей подряд).',
      '⚠️ ТОЛЬКО БУМАГА, в реал не допускается. Мартингейл — высоко-дисперсная механика: Монте-Карло показал высокий риск разорения даже при крае. Исправления делают её менее убыточной и ограничивают урон, но это эксперимент для проверки гипотезы о времени входа, а не источник дохода. Судим по форвард-выборке.',
    ],
  },
  {
    slug: 'mart7',
    engine: 'martingale-fib',
    realEligible: false,
    title: 'Мартингейл-7 шагов (1,1,2,3,5,8,13) 🧪',
    tagline: 'Копия мартингейла, Фибоначчи 7 шагов [1,1,2,3,5,8,13], после 7 проигрышей сброс. Макс. ставка на серии $13. Только бумага.',
    statusEnv: 'PREDICT_MART7_STATUS_PATH',
    statusFile: 'predict-mart7-status.json',
    liveFile: 'predict-live-mart7.json',
    showStakeCol: true,
    hasLive: true,
    description: [
      'То же ядро (андердог коэф 3.0–3.5, окно входа 60–180с, стоп-серия на 10), прогрессия Фибоначчи ограничена 7 шагами: [1,1,2,3,5,8,13]. После 7 проигрышей подряд сброс к базе $1. Максимум на серии — $13.',
      'Часть полного A/B-ряда длины прогрессии (3/4/5/6/7/8 шагов) — самый агрессивный после основного (8 шагов).',
      '⚠️ ТОЛЬКО БУМАГА, в реал не допускается.',
    ],
  },
  {
    slug: 'mart6',
    engine: 'martingale-fib',
    realEligible: false,
    title: 'Мартингейл-6 шагов (1,1,2,3,5,8) 🧪',
    tagline: 'Копия мартингейла, Фибоначчи 6 шагов [1,1,2,3,5,8], после 6 проигрышей сброс. Макс. ставка на серии $8. Только бумага.',
    statusEnv: 'PREDICT_MART6_STATUS_PATH',
    statusFile: 'predict-mart6-status.json',
    liveFile: 'predict-live-mart6.json',
    showStakeCol: true,
    hasLive: true,
    description: [
      'То же ядро (андердог коэф 3.0–3.5, окно входа 60–180с, стоп-серия на 10), прогрессия Фибоначчи ограничена 6 шагами: [1,1,2,3,5,8]. После 6 проигрышей подряд сброс к базе $1. Максимум на серии — $8.',
      'Часть полного A/B-ряда длины прогрессии (3/4/5/6/7/8 шагов).',
      '⚠️ ТОЛЬКО БУМАГА, в реал не допускается.',
    ],
  },
  {
    ...
    engine: 'martingale-fib',
    realEligible: false,
    title: 'Мартингейл-5 шагов (1,1,2,3,5) 🧪',
    tagline: 'Копия мартингейла, Фибоначчи 5 шагов [1,1,2,3,5], после 5 проигрышей сброс. Макс. ставка на серии $5. Только бумага.',
    statusEnv: 'PREDICT_MART5_STATUS_PATH',
    statusFile: 'predict-mart5-status.json',
    liveFile: 'predict-live-mart5.json',
    showStakeCol: true,
    hasLive: true,
    description: [
      'То же ядро (андердог коэф 3.0–3.5, окно входа 60–180с, стоп-серия на 10), прогрессия Фибоначчи ограничена 5 шагами: [1,1,2,3,5]. После 5 проигрышей подряд сброс к базе $1. Максимум на серии — $5.',
      'Часть полного A/B-ряда длины прогрессии (3/4/5/6/7/8 шагов) — ищем баланс между отыгрышем серии и просадкой.',
      '⚠️ ТОЛЬКО БУМАГА, в реал не допускается.',
    ],
  },
  {
    ...
    engine: 'martingale-fib',
    realEligible: false,
    title: 'Мартингейл-4 шага (1,1,2,3) 🧪',
    tagline: 'Копия мартингейла, но Фибоначчи ограничена 4 шагами [1,1,2,3] — после 4 проигрышей сброс к базе. Менее агрессивный разгон ставки. Только бумага.',
    statusEnv: 'PREDICT_MART4_STATUS_PATH',
    statusFile: 'predict-mart4-status.json',
    liveFile: 'predict-live-mart4.json',
    showStakeCol: true,
    hasLive: true,
    description: [
      'То же, что основной мартингейл (вход на андердога коэф 3.0–3.5, окно входа 60–180с, стоп-серия на 10), НО прогрессия Фибоначчи укорочена до 4 шагов: [1,1,2,3]. После 4 проигрышей подряд ставка сбрасывается к базе $1 и серия начинается заново.',
      'Зачем: короткая прогрессия = меньший разгон ставки = ниже риск глубокой просадки, но и слабее «отыгрыш» серии. Это A/B-сравнение с основным (8 шагов) и 3-шаговым вариантом — какая длина прогрессии даёт лучший баланс роста и риска.',
      '⚠️ ТОЛЬКО БУМАГА. Мартингейл высоко-дисперсен; эксперимент для сравнения длины прогрессии, не источник дохода.',
    ],
  },
  {
    slug: 'mart3',
    engine: 'martingale-fib',
    realEligible: false,
    title: 'Мартингейл-3 шага (1,1,2) 🧪',
    tagline: 'Копия мартингейла с самой короткой Фибоначчи — 3 шага [1,1,2], после 3 проигрышей сброс. Минимальный разгон ставки. Только бумага.',
    statusEnv: 'PREDICT_MART3_STATUS_PATH',
    statusFile: 'predict-mart3-status.json',
    liveFile: 'predict-live-mart3.json',
    showStakeCol: true,
    hasLive: true,
    description: [
      'То же, что основной мартингейл (вход на андердога коэф 3.0–3.5, окно входа 60–180с, стоп-серия на 10), НО прогрессия Фибоначчи укорочена до 3 шагов: [1,1,2]. После 3 проигрышей подряд ставка сбрасывается к базе $1.',
      'Самый консервативный из трёх по разгону ставки: максимум на серии — $2 вместо $3 (4 шага) или $21 (8 шагов). Минимальная просадка на плохой серии, но и минимальный «отыгрыш». A/B-сравнение длины прогрессии.',
      '⚠️ ТОЛЬКО БУМАГА. Эксперимент для сравнения длины прогрессии, не источник дохода.',
    ],
  },
  {
    slug: 'egprog',
    ...
    tagline: '⏸ НА ПАУЗЕ (слот фидов освобождён под «Андердог+Келли»). Эндшпиль + размер ставки по дробному Келли (1/4) от банка $100',
  ...
  },
  {
    ...
    engine: 'endgame-edge',
    realEligible: true,
    title: 'Эндшпиль · цена ≤0.70 + перевес (f>0)',
    tagline: '⏸ НА ПАУЗЕ (слот фидов освобождён под A/B мартингейлов). Средний фильтр цены: вход при цене ≤0.70 и крае f>0',
    ...
    hasLive: true,
    description: [
      'Средняя ступень градиента по цене входа. Версия с фильтром «цена ≤0.80 + f>0» дала минус (−4.7пп на 84 сделках): одного условия f>0 не хватило, край всё ещё уходил в зону 0.71–0.80, где нужен винрейт >77%. Поэтому докрутили: теперь вход только при цене ≤0.70 И положительном крае f>0.',
      'Зачем именно ≤0.70. Разбор входов показал: убыточная зона — это цена 0.76–0.80 (нужен WR 77%+, которого нет). Порог ≤0.70 отсекает её и оставляет коэффициенты от 1.43 и выше, где асимметрия выплат уже в нашу пользу. Это компромисс: сделок больше, чем у строгой комбо (≤0.65), но без заведомо убыточной зоны.',
      'Зачем три варианта. Получается чистый градиент по главному рычагу — цене входа: базовая eglate (≤0.80) → egedge (≤0.70) → комбо (≤0.65 + z≥1.5 + сессия). Сравнив их на одних рынках, увидим, насколько строгим должен быть фильтр цены, чтобы PnL рос стабильно.',
      'Размер ставки — дробный Келли (1/4) от банка: ¼ · f · банк, само-калибровка по фактическому винрейту, потолок ~30% банка, минимум $1. f = p − (1−p)/b; вход только при f>0. Бот ставит рыночные ордера, минимум Polymarket — $1, поэтому работает и с малым депозитом.',
    ],
  },
  {
    slug: 'egcombo',
    ...
    tagline: '⏸ НА ПАУЗЕ (слот фидов освобождён под A/B мартингейлов). Три фильтра: цена ≤0.65 + z≥1.5 + сессия 00–13 UTC',
