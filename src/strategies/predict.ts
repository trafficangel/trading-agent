...
const MART_MAX_STAKE = [1, 1, 2, 3, 5, 8]; // макс. ставка по капу (индекс cap-1)

const STRATEGIES: StrategyDef[] = [
  ...[3, 4, 5, 6].map(
    (n): StrategyDef => ({
      slug: `mart${n}`,
      title: `Мартингейл · кап ${n}`,
      tagline: `Аутсайдер (коэф 3.0–3.5) + Фибоначчи, сброс после ${n} проигрышей подряд`,
      statusEnv: `PREDICT_MART${n}_STATUS_PATH`,
      statusFile: `predict-mart${n}-status.json`,
      liveFile: `predict-mart${n}-live.json`,
      showStakeCol: true,
      hasLive: true,
      description: [
        'Ставим на сторону, чей коэффициент попадает в диапазон 3.0–3.5 (рынок считает её аутсайдером с шансом ~30%, выплата ~3:1). Размер ставки по Фибоначчи от $1: 1, 1, 2, 3, 5, 8… После проигрыша — следующий шаг последовательности, после выигрыша — сброс к $1.',
        `Особенность варианта: серия ограничена ${n} шагами — после ${n} проигрышей подряд сбрасываемся к $1, фиксируя убыток серии. Максимальная ставка ≈ $${MART_MAX_STAKE[n - 1]}. Чем меньше кап, тем меньше риск глубокой просадки, но тем чаще фиксируются мелкие убытки.`,
        'Это A/B-тест параметра «потолок серии» (3 / 4 / 5 / 6). Честно: у ставки на аутсайдера по справедливому коэффициенту матожидание ≈ 0 (минус спред), поэтому стабильного роста не даёт ни один кап — сравниваем, какой ведёт себя ровнее и теряет меньше.',
      ],
    }),
  ),
  {
    ...
    hasLive: true,
    description: [
      'Эта стратегия не угадывает, куда пойдёт BTC. Она реагирует на почти определившийся исход. В последние ≤90 секунд раунда считаем, насколько далеко цена ушла от цели относительно её типичных колебаний за оставшееся время (z-оценка). Чем больше z, тем выше вероятность, что лидирующая сторона удержится до закрытия.',
      'Когда z большой (вероятность удержания ~97%+), а лучший ask этой стороны всё ещё заметно дешевле справедливой вероятности — берём её. Идея: ордербук тонкого 5-минутного рынка не успевает переоцениться под уже сложившийся факт; покупаем выигрышную сторону с дисконтом и держим до резолюции.',
      'Эдж здесь — скорость и лаг книги, а не предсказание направления. Запас (минимум 3¢ над справедливой ценой) заложен на комиссию, спред и ошибку оценки волатильности. Если рынок уже переоценился — пропускаем раунд. Честно: положительность эджа подтверждается только статистикой, поэтому проверяем так же строго, как остальные.',
    ],
  },
  {
    slug: 'fav',
    title: 'Ставка на фаворита',
    tagline: 'Зеркало мартингейла: берём фаворита (коэф 1.18–1.67), плоская ставка',
    statusEnv: 'PREDICT_FAV_STATUS_PATH',
    statusFile: 'predict-fav-status.json',
    liveFile: 'predict-fav-live.json',
    showStakeCol: true,
    hasLive: true,
    description: [
  ...
  },
  {
    ...
    hasLive: true,
    description: [
      'Стратегия принимает решения по сигналам индикаторов LuxAlgo с минутного графика BTC. Алерты из TradingView приходят на наш сервер вебхуком в реальном времени — на закрытии каждого 1-минутного бара. Мы храним последние сигналы и каждые полсекунды пересчитываем по ним «направленный перевес» (bias). Размер ставки фиксированный — $15 на раунд; позицию держим до конца 5-минутного окна, без выхода в середине.',
      'Какие сигналы участвуют. Подтверждение вверх/вниз (Confirmation) — основной триггер направления; усиленное подтверждение (Confirmation+) весит вдвое. Тренд-фон — Trend Catcher (основной) и Trend Tracer (быстрый, послабее) — задают текущее направление тренда. Развороты (Reversal / Contrarian) — лёгкий контр-сигнал. У каждого сигнала есть знак (вверх = плюс, вниз = минус) и вес; всё складывается в один перевес.',
      'Свежесть сигналов. Триггеры-события (подтверждения и развороты) «живут» 3 минуты и плавно затухают — старый сигнал влияет слабее свежего. Тренд-фон (Catcher / Tracer) держится дольше, до 10 минут, как устойчивое направление между событиями.',
      'Условия входа — выполниться должны все три одновременно:',
      '1) Уверенность направления: суммарный модуль перевеса ≥ 2. Например, одно усиленное подтверждение (вес 2) — или обычное подтверждение плюс тренд в ту же сторону.',
      '2) Свежий повод: за последние 3 минуты было хотя бы одно событие (подтверждение или разворот). На одном застывшем тренде, без свежего триггера, мы не входим.',
      '3) Ценность и время: цена нужной стороны в полосе 0.35–0.65 (рынок ещё не задрал её вверх — иначе выплата мала); до закрытия раунда осталось достаточно времени (входим в окне примерно от 4 минут до 5 секунд до конца); в стакане есть нужная ликвидность.',
      'Когда раунд пропускаем. Нет свежих сигналов (фид молчит); перевес слабый (модуль < 2); цена нужной стороны вне полосы 0.35–0.65 (рынок уже согласен с направлением — платить будет мало); не хватает времени или ликвидности. Принцип простой: лучше пропустить, чем войти на шуме.',
      'Почему ставка плоская. Пока нет доказательства, что более сильный перевес чаще приводит к выигрышу, наращивать ставку под «уверенность» — значит обманывать себя. Сначала набираем статистику; если по данным сильный перевес реально выигрывает чаще — тогда введём аккуратный размер по уверенности (дробный Келли с потолком). До тех пор — честные плоские $15.',
