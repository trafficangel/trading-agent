...
type ProfitGateStatus = {
  ready?: boolean;
  noGo?: boolean;
  decision?: 'GO' | 'NO_GO' | 'OBSERVE';
  reasons?: string[];
  noGoReasons?: string[];
  requirements?: {
    minSamples?: number;
    noGoAfterMs?: number;
    minTradesByDeadline?: number;
  };
  metrics?: {
    samples?: number;
    trades?: number;
...
function activeRows(rows: Opportunity[], triggerBps: number): string {
  ...
  if (!profitable.length) {
    return `<div class="va-empty">Сейчас нет расхождений с потенциалом не ниже ${pctFromBps(triggerBps)} на $1,000 после комиссий — при условии последующего схождения цен.</div>`;
  }
  const now = Date.now();
  return `<div class="va-table" data-va-pager="active" data-page-size="20"><table><thead><tr>
    ...
    <th>VWAP $1,000</th><th>Доступная глубина</th><th>Возраст стакана</th><th>Потенциал при схождении</th>
...
function historyRows(rows: Opportunity[]): string {
  ...
  if (!profitable.length) return '<div class="va-empty">Завершённых окон с положительным потенциалом при схождении пока нет.</div>';
...
function candidateRouteRows(
  ...
  const makerRows = [
    ...
    const state = gate?.noGo
      ? '<b class="neg">NO-GO</b>'
      : (gate?.ready ?? readiness?.ready)
      ...
      : maker?.pair
        ...
        : samples > 0
          ? '<b>НАБЛЮДЕНИЕ</b>'
          : '<b>ЖДЁМ FILL</b>';
    const blocker = gate?.noGo
      ? esc(gate.noGoReasons?.[0] ?? 'маршрут закрыт')
      : !maker?.enabled
      ? 'выключен'
  ...
  return pageShell(
    ...
    <div class="va-wrap va-compact">
      ...
      <section class="va-panel">
        ...
        <p class="va-compact-note">Строгая эпоха с 07-30 03:48 UTC: maker-fill засчитывается только после публично наблюдаемого объёма на цене или через неё, достаточного для очереди и всей нашей заявки. Более ранние оптимистичные fills исключены.</p>
  ...
  return pageShell(
    ...
    <div class="va-wrap">
      ...
      <div class="va-head"><div>
        ...
        <p>Фиксирует расхождения, исполнимые на $1,000. Показанный net — это потенциал только при последующем схождении цен: из стартового spread уже вычтены четыре taker-комиссии и защитный буфер. Это не гарантированная прибыль; факт проверяют execution-shadow и реальный журнал ниже.</p>
      </div><div class="va-engine ${isLive ? 'live' : ''}"><i></i>${isLive ? 'РАБОТАЕТ' : 'НЕТ СВЕЖИХ ДАННЫХ'}</div></div>

      <div class="va-cards">
        <div class="va-card"><small>Потоки</small><b>${connected}/${VENUES.length}</b><em>Lighter · Hyperliquid · Paradex · Polymarket · Extended · Aster · Pacifica · GRVT · edgeX · Binance · Bybit</em></div>
        ...
        <div class="va-card"><small>Кандидаты сейчас</small><b class="${profitableActive.length ? 'pos' : ''}">${profitableActive.length}</b><em>$1,000 потенциал &gt; ${pctFromBps(triggerBps)}</em></div>
        <div class="va-card"><small>Окон потенциала завершено</small><b class="${Number(summary.viable ?? 0) > 0 ? 'pos' : ''}">${Number(summary.viable ?? 0)}</b><em>из ${Number(summary.closed ?? 0)} наблюдений</em></div>
        <div class="va-card"><small>Медиана потенциала</small><b class="pos">${pctFromBps(summary.medianViablePeakNetBps)}</b><em>при схождении · лучший ${pctFromBps(summary.maxPeakNetBps)}</em></div>
        <div class="va-card"><small>Жизнь расхождения</small><b>${duration(summary.medianViableDurationMs)}</b><em>$1,000 · до схождения не прибыль</em></div>
      ...
      <section class="va-panel"><div class="va-panel-head"><h2>Кандидаты на схождение сейчас</h2><span>$1,000 potential net &gt; ${pctFromBps(triggerBps)} · автообновление 5 сек</span></div>
      ...
      <section class="va-panel va-shadow-panel">
        ...
        <p>Каждый probe начинается только когда ожидаемый net возврата к нормальной базе ≥ ${pctFromBps(executionShadow?.config?.entryNetBps)}. Он рассчитан по четырём исполнимым ценам: текущий входной basis плюс медианный basis обратного выхода, затем вычтены четыре taker-комиссии и ${pctFromBps(executionShadow?.config?.executionBufferBps, false)} execution-буфера. Поэтому постоянный абсолютный spread и bid/ask обеих бирж не выдаются за edge. Baseline — медиана последних ${duration(executionShadow?.config?.basisWindowMs ?? 30 * 60_000)}, последние ${duration(executionShadow?.config?.basisExcludeMs ?? 5_000)} исключены, а до накопления ${Number(executionShadow?.config?.basisMinSamples ?? 120)} наблюдений вход запрещён. После этого требуются ${Number(executionShadow?.config?.entryConfirmations ?? 3)} независимых свежих подтверждения. Затем фиксируется $${Number(executionShadow?.config?.notionalUsd ?? 500)} VWAP и учитывается funding ${pctFromBps(executionShadow?.config?.fundingBpsPerHour, false)}/час. Выход допускается после ${Number(executionShadow?.config?.exitConfirmations ?? 3)} свежих снимков с реальным модельным PnL ≥ ${pctFromBps(executionShadow?.config?.exitNetBps)}, затем применяется измеренная exit latency. Gate: минимум ${Number(shadowGate?.requiredSamples ?? 20)} подтверждённых модельных входов и PASS ≥ ${pct(shadowGate?.requiredPassPct)}.</p>
    ...
    : `Для ${esc(liveStatus?.route ?? 'Extended ↔ Lighter')} expected net считается как отклонение текущей базы от собственной медианы за ${duration(Number(liveStatus?.basisGate?.windowSeconds ?? 1800) * 1000)} минус полный round-trip cost ${plainPct(liveStatus?.basisGate?.roundTripCostPct ?? .07)}. Обе taker-ноги отправляются параллельно только при expected net ≥ ${plainPct(liveStatus?.entryNetPct ?? .10)}; недавние ${duration(Number(liveStatus?.basisGate?.excludeRecentSeconds ?? 5) * 1000)} не входят в baseline. Знак постоянного абсолютного spread на допуск больше не влияет.`} Обычный выход требует ${Number(liveStatus?.exitConfirmations ?? 3)} последовательных снимков с исполнимым net PnL ≥ ${plainPct(liveStatus?.exitMinProfitPct ?? .03)}. Прямой защитный выход срабатывает при projected net ≤ −${money(liveStatus?.maxLossUsd ?? .05)}.</p>
        <div class="va-table" data-va-pager="live-trades" data-page-size="20"><table><thead><tr>
      ...
      <section class="va-panel"><h2>Прибыльные типы маршрутов</h2>
        <p>Показываются маршруты, где стартовый $1,000 VWAP оставлял потенциал выше ${pctFromBps(triggerBps)} после четырёх taker-комиссий и ${pctFromBps(status?.executionBufferBps, false)} буфера, если межбиржевая разница затем сходится. Реализованный результат подтверждается только таблицами shadow/live.</p>
      ...
      <section class="va-panel"><h2>История окон потенциала</h2>${historyRows(status?.recentClosed ?? [])}</section>
