...
const CONNECTION_STALE_MS = Number(process.env.HFT_CONNECTION_STALE_MS ?? 30_000);
...
const connectionState: Record<
  Venue,
  { connected: boolean; messages: number; reconnects: number; lastMessageAt: number }
> = {
...
function openHour(now: number): void {
  ...
  gzip.pipe(file);
  gzip.on('error', (error) => {
    console.error('leadlag gzip error', error);
  });
  file.on('error', (error) => {
    console.error('leadlag file error', error);
  });
}
...
function priceFlowTuple(flow: Flow): number[] {
  return [...flow.prices.entries()]
    .sort((a, b) => a[0] - b[0])
    .flatMap(([price, signedSize]) => [price, signedSize]);
}
...
function sample(): void {
  ...
  for (const state of states.values()) {
    const ready = (Object.values(state.books) as Book[]).every(
      (book) => book.bid > 0 && book.ask > book.bid && now - book.receivedAt <= STALE_MS,
    );
    if (ready) {
      const row = {
        v: 1,
        ...
        f: [
          ...flowTuple(state.flows.hl),
          ...flowTuple(state.flows.binance),
...
function writeStatus(): void {
  ...
  const status = {
    ...
    freshnessMs: Object.fromEntries(
...
function connect(
  ...
  ws.addEventListener('message', (event) => {
    ...
    try {
      ...
      console.warn(`leadlag ${venue} parse`, error);
    }
  });
  ...
  connect(
    ...
    (ws) => {
      for (const { coin } of MARKETS) {
        for (const type of ['bbo', 'l2Book', 'trades'])
    ...
    (payload, receivedAt) => {
      ...
      if (message.channel === 'bbo') {
        const data = message.data as {
          coin?: string;
          time?: number;
          bbo?: Array<{ px?: string; sz?: string; n?: number } | null>;
        };
        ...
        Object.assign(state.books.hl, {
          bid: finite(bid.px),
          ask: finite(ask.px),
          bidSize: finite(bid.sz),
          askSize: finite(ask.sz),
          exchangeAt: finite(data.time),
          receivedAt,
        });
        ...
        Object.assign(state.books.hl, {
          bid: finite(bids[0].px),
          ask: finite(asks[0].px),
          bidSize: finite(bids[0].sz),
          askSize: finite(asks[0].sz),
        ...
        for (const trade of message.data as Array<{
          coin?: string;
          side?: string;
          px?: string;
          sz?: string;
    ...
    (payload, receivedAt) => {
      ...
      if (data.e === 'bookTicker') {
        Object.assign(state.books.binance, {
          ...
          bid5: finite(data.B),
          ask5: finite(data.A),
          exchangeAt: finite(data.T ?? data.E),
          receivedAt,
        });
      } else if (data.e === 'aggTrade') {
        appendFlow(
          state.flows.binance,
          data.m === true ? 'sell' : 'buy',
          finite(data.p),
          finite(data.q),
        );
      }
    ...
    (payload, receivedAt) => {
      ...
      if (message.topic?.startsWith('orderbook.1.')) {
        ...
        for (const trade of message.data as Array<{
          ...
          const coin = trade.s ? bySymbol.get(trade.s) : null;
          const state = coin ? states.get(coin) : null;
          if (state)
            appendFlow(
              state.flows.bybit,
              trade.S === 'Buy' ? 'buy' : 'sell',
              finite(trade.p),
              finite(trade.v),
            );
        }
...
const watchdogTimer = setInterval(() => {
  const now = Date.now();
  if (now - startedAt < WATCHDOG_GRACE_MS) return;
  for (const venue of ['hl', 'binance', 'bybit'] as const) {
    const connection = connectionState[venue];
    if (connection.connected && now - connection.lastMessageAt <= CONNECTION_STALE_MS) continue;
    console.error(
      `leadlag watchdog stale ${venue}: connected=${connection.connected} ageMs=${now - connection.lastMessageAt}`,
    );
    shutdown(`STALE_${venue.toUpperCase()}`);
    return;
  }
}, 5_000);
...
console.warn(
  `leadlag collector started: ${MARKETS.map((m) => m.coin).join(',')} @ ${SAMPLE_MS}ms -> ${DATA_DIR}`,
);
