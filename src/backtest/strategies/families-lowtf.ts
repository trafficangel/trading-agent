...
export function zscoreMr(symbol: string, tf: string, period = 50, zThr = 2, trendEma = 0, slPct = 0.05): CustomStrategy {
  ...
  return { id: `z${period}_${zThr}t${trendEma}-${symbol}-${tf}`, code: 'zscore', name: `${symbol} Z-score`, symbol, timeframe: tf, slPct, warmup: Math.max(period + 1, trendEma + 1), description: `Z-score(${period}) reversion ±${zThr}σ${trendEma ? ` +EMA${trendEma}` : ''}`,
...
export function keltnerMr(symbol: string, tf: string, emaP = 20, atrP = 10, mult = 2, trendEma = 0, slPct = 0.05): CustomStrategy {
  ...
  return { id: `kelt${emaP}_${atrP}_${mult}t${trendEma}-${symbol}-${tf}`, code: 'keltner', name: `${symbol} Keltner`, symbol, timeframe: tf, slPct, warmup: Math.max(emaP, atrP, trendEma) + 1, description: `Keltner(${emaP},${atrP},${mult}) reversion${trendEma ? ` +EMA${trendEma}` : ''}`,
